...
public class ListPositionCtrl : MonoBehaviour
{
	...
	void storeMousePosition()
	{
		...
		else if ( Input.GetMouseButton(0) )
		{
            ...
            if (lastInputWorldPos != currentInputWorldPos)
                isMove = true;
    ...
    public void Cheat()
    {
        ...
        lastInputWorldPos = Camera.main.ScreenToWorldPoint(Vector3.zero);
        currentInputWorldPos = Camera.main.ScreenToWorldPoint(Vector3.one);
        //theo chieu Y
        //deltaInputWorldPos = new Vector3( 0.0f, currentInputWorldPos.y - lastInputWorldPos.y, 0.0f );
	...
	float findDeltaPositionY()
	{
		...
		foreach ( ListBox listBox in listBoxes )
		{
			deltaPosY = centerPosY - listBox.transform.position.x;
